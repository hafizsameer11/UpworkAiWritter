   ...
   <div class="row justify-content-center mt-4">
      <div class="col-md-6">
        <div class="card shadow-sm rounded-3 overflow-hidden">
          ...
          <div class="card-body">
            <h6><strong>Niche : </strong> {{ $project->niche->name }}</h6>
            <h6><strong>URL : </strong> {{ $project->project_url }}</h6>
            <p>{{ $project->description }}</p>
          </div>
          <div class="card-footer py-4">
            <div class="d-flex align-items-center gap-4">
               <a href="{{route('niches.edit', $project->id)}}" class="btn btn-primary">
                 <i class="fas fa-edit"></i> Edit
               </a>
               <button class="btn delete_Btn btn-danger ml-2" data-id="{{$project->id}}" data-name="{{$project->name}}">
